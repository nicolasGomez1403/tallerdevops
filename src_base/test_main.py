...
from logica import calcular_indicadores_rendimiento
...
def test_calcular_indicadores_lista_vacia():
    resultado = calcular_indicadores_rendimiento([])
    assert resultado == {
...
def test_calcular_indicadores_notas_variadas():
    notas = [3.5, 4.0, 2.0, 5.0, 1.5]
    resultado = calcular_indicadores_rendimiento(notas)
    ...
    assert resultado["tasa_aprobacion"] == 60.0
    assert resultado["rango_calificaciones"] == 3.5

def test_calcular_indicadores_todos_aprobados():
    notas = [4.5, 3.8, 5.0]
    resultado = calcular_indicadores_rendimiento(notas)
    ...
    assert resultado["tasa_aprobacion"] == 100.0
    assert resultado["rango_calificaciones"] == 1.2

def test_calcular_indicadores_todos_reprobados():
    notas = [1.0, 2.5, 2.8]
    resultado = calcular_indicadores_rendimiento(notas)
    ...
    assert resultado["tasa_aprobacion"] == 0.0
    assert resultado["rango_calificaciones"] == 1.8
